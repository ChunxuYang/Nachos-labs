...
public:
	...
	// internal emulation routines -- DO NOT call these.
	void WriteDone() { semaphoreWrite->V(); }; // internal routines to signal I/O completion
	void CheckCharAvail();
	void ReadAvail() { semaphoreRead->V(); };

	static void SynchConsoleRead(int sc)
	{
		SynchConsole *console = (SynchConsole *)sc;
		console->ReadAvail();
	}
	static void SynchConsoleWrite(int sc)
	{
		SynchConsole *console = (SynchConsole *)sc;
		console->WriteDone();
	}
